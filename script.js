...
// MODELO DE QUESTÕES
// =====================
// Adicionar 15 questões abaixo (modelo)
const questions = [
  {
    ...
    options: ["Pipeline.", " Peer to Peer.", "Cliente-Servidor", "MVC Model-View-Controller", " Mestre-escravo"],
    correct: 3,
  },
  {
  ...
  },
  {
    ...
    options: ["Inserção de imagem", "Inserção de link", "Definição de parágrafo", "Definição de título ", " Definição de tabela."],
    correct: 1, // (número da alternativa correta começando em 0)
},
{
    question: " Pergunta 4: HyperText Markup Language HTML) é uma linguagem que nos permite criar conteúdo na web que pode ser renderizado por navegadores como o Google Chrome, Microsoft Edge e o Safari. A respeito dessa linguagem, analise as afirmativas abaixo e marque a alternativa correta  I. Documentos HTML são formados por marcadores tags que são delimitados pelos caracteres "<" e ">". Além dos marcadores, temos nos documentos HTML os elementos. Eles são formados pelos marcadores que podem possuir atributos valores e elementos filhos.II.Os marcadores em HTML são case sensitive.III.Em sua versão 5, o HTML trouxe uma série de novos marcadores para elementos. São exemplos destes novos marcadores: , , e .",
    options: ["Apenas as afirmativas I e III estão corretas", " Apenas a afirmativa I está correta.  ", ")Apenas a afirmativa III está correta.  ", "Apenas as afirmativas II e III estão corretas.", "Apenas a afirmativa II está correta."],
    correct: 0, // (número da alternativa correta começando em 0)
},
{
    ...
    options: [" margin: 16px; ", "border-width: 16px;", "word-spacing: 16px; ", "letter-spacing: 16px;", "padding: 16px;"],
    correct: 4, // (número da alternativa correta começando em 0)
},
{
...
},
{
    ...
    options: ["RGB- RGB com porcentagem - LAB", "Palavras-chave - RGB com porcentagem - números hexadecimais com canal alpha.", "CMYK-RGB - palavraschave.", "HSL-RGB - números hexadecimais.", "RGBA-HSLA - números decimais."],
     correct: 3, // (número da alternativa correta começando em 0)
},
{
    question: "Pergunta 8 : JavaScript é uma linguagem de programação interpretada estruturada de script muito usada para programação FrontEnd. Selecione o método em JavaScript que remove de um vetor o primeiro elemento e o retorna como resultado. ",
     options: ["push()", "shift()", "pop()", "join()", "reverse()"],
    correct: 1, // (número da alternativa correta começando em 0)
},
{
    ...
    options: ["responseString ", "ServerResponse", "ResponseText ", "JSONResponse ", "JSONResponseString"],
    correct: 2, // (número da alternativa correta começando em 0)
},
{
    ...
    options: ["echo $estados[1; ", "print( $estados[1 );", "print_r( $estados[1 );", "print( $estados );", "echo $estados[0]"],
    correct: 4, // (número da alternativa correta começando em 0)
},
{
    ...
    options: [" $cadastro =array(''CPF'':''1234567890'',''Nome'':''Pedro'' ,''Cidade'':''Cascavel'');  ", "$cadastro =array(''CPF'':=''1234567890'',''Nome'':=''Pedro'' ,''Cidade'':=''Cascavel'';", "$cadastro =array(''CPF''=''1234567890'', ''Nome''=''Pedro'' , ''Cidade''=''Cascavel''); ", ")$cadastro =array(''CPF''''1234567890'',''Nome''''Pedro'' ,''Cidade''''Cascavel'');  ", "$cadastro =array(''CPF''|''1234567890'',''Nome''|''Pedro'' ,''Cidade''|''Cascavel''); "],
    correct: 3, // (número da alternativa correta começando em 0)
},
{
    ...
    options: ["Exit. ", "Quit", "Break.", "halt.", "fork."],
    correct: 2, // (número da alternativa correta começando em 0)
},
{
    ...
    options: ["HTML $_FORM, $_HTTP ", "$_GET, $_POST ", "$_SUPER_GET, $_SUPER_POST", "$_FORM, $_RESPONSE", "$_SUPER_FORM, $_SUPER_HTTP"],
    correct: 1, // (número da alternativa correta começando em 0)
},
{
    question: "Pergunta 14:Qual é a forma correta de conectar o PHP a um banco de dados MySQL? ",
    options: ["$conecta = mysql(HOST, LOGIN, SENHA) or print (mysql_error());", " $conecta = mysql_connect(HOST, LOGIN, SENHA) or print (mysql_error());", "$conecta = connect(HOST, LOGIN, SENHA) or print (error()); ", "$conecta = mysql_connect(HOST, LOGIN, SENHA) or print (error());", "$conecta = new MySQLHOST, LOGIN, SENHA) or die Mysqlerror());"],
    correct: 1, // (número da alternativa correta começando em 0)
},
{
    ...
    options:["GET, POST e DATAREQUEST", "doGet, doPost e processRequest ", "$GET_DATA, $POST_DATA e $REQUEST_DATA", "$_DOGET, $_DOPOST e $_DOREQUEST ", "$_GET, $_POST e $_REQUEST"],
    correct: 1, // (número da alternativa correta começando em 0)
},
  // Adicione até a 15ª questão...
];
...
function selectOption(index) {
  ...
  if (selectedOption) return; // Impede múltiplas seleções
  ...
  if (index === q.correct) correctAnswers++;
